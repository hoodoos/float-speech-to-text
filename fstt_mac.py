"""
Headless macOS port of float-speech-to-text.

- Global hotkey toggles recording (default: Ctrl+Shift)
- Menubar icon shows state (idle/recording/processing)
- Menu: toggle LLM polish, auto-paste, loading dots, quit
...
Requires macOS Accessibility permission (System Settings → Privacy &
Security → Accessibility) for the Python/terminal binary that runs this
script. Without it, global hotkey capture and synthetic keystrokes
silently fail.
"""
    ...
    def load(self):
        with self._lock:
            if self._model is None:
                ...
                t0 = time.time()
                # CoreML execution provider fails on gigaam-v3-e2e-rnnt
                # (returns -1 on Espresso prediction). Force CPU on macOS.
...
def _release_mods():
    ...
    synthetic keystrokes run while the user still holds the hotkey."""
    for k in _ALL_MOD_KEYS:
        try:
            _kb.release(k)
        except Exception:
            pass
...
def _type_text(s: str, delay: float):
    _release_mods()
    if delay <= 0:
        _kb.type(s)
        return
...
# ---------- Hotkey (modifiers-only, edge-triggered) ----------
_MOD_ALIASES = {
...
def _parse_hotkey(spec: str) -> set:
    ...
    for part in spec.split("+"):
        ...
        if p in _MOD_ALIASES:
            ...
            # TODO: support non-modifier keys in hotkey combos (e.g. ctrl+shift+space)
            log.warning("hotkey part %r is not a modifier — ignored", p)
    if not mods:
        mods = {"ctrl", "shift"}
    return mods
...
class App(rumps.App):
    def __init__(self):
        super().__init__("fstt", title=_PHASE_TITLE[Phase.IDLE], quit_button=None)
        ...
        self._mi_llm = rumps.MenuItem("LLM polish", callback=self._toggle_llm)
        self._mi_llm.state = 1 if self.llm_enabled else 0
        self._mi_paste = rumps.MenuItem("Auto-paste", callback=self._toggle_paste)
        self._mi_paste.state = 1 if self.auto_paste else 0
        self._mi_placeholder = rumps.MenuItem("Loading dots", callback=self._toggle_placeholder)
        self._mi_placeholder.state = 1 if self.placeholder else 0
    ...
    def _refresh_icon(self):
        self.title = _PHASE_TITLE[self.phase]
    ...
    def _toggle_llm(self, sender):
        ...
        sender.state = 1 if self.llm_enabled else 0
        log.info("LLM enabled: %s", self.llm_enabled)

    def _toggle_paste(self, sender):
        self.auto_paste = not self.auto_paste
        sender.state = 1 if self.auto_paste else 0
        log.info("auto-paste: %s", self.auto_paste)

    def _toggle_placeholder(self, sender):
        self.placeholder = not self.placeholder
        sender.state = 1 if self.placeholder else 0
    ...
    def _check_permissions(self):
        # 1. Accessibility — for posting synthetic keystrokes (paste/type).
        try:
            from ApplicationServices import AXIsProcessTrustedWithOptions
            ax_ok = AXIsProcessTrustedWithOptions(
                {"AXTrustedCheckOptionPrompt": True}
            )
            log.info("Accessibility: %s", "granted" if ax_ok else "NOT GRANTED")
        except Exception as e:
            log.error("accessibility check failed: %s", e)
            ax_ok = False

        # 2. Input Monitoring — for capturing the global hotkey. Request
        # explicitly via IOKit so the system prompt appears reliably.
        try:
            ...
            im_ok = bool(iokit.IOHIDRequestAccess(1))
            log.info("Input Monitoring: %s", "granted" if im_ok else "NOT GRANTED")
        except Exception as e:
            log.error("input-monitoring check failed: %s", e)
            im_ok = False

        # 3. Microphone — for audio capture. Request explicitly via
        # AVFoundation; the completion handler fires async once the user
        # responds, so we just fire-and-forget here.
        mic_ok = False
        try:
            ...
            mic_ok = status == AVAuthorizationStatusAuthorized
            log.info(
                "Microphone: %s (status=%s)",
                "granted" if mic_ok else "NOT GRANTED", status,
            )
            if not mic_ok:
                AVCaptureDevice.requestAccessForMediaType_completionHandler_(
                    AVMediaTypeAudio,
                    lambda granted: log.info(
                        "Microphone prompt result: %s",
                        "granted" if granted else "denied",
                    ),
                )
        except Exception as e:
            log.error("microphone check failed: %s", e)

        if not (ax_ok and im_ok and mic_ok):
            log.warning(
                "Missing permissions. Approve each prompt, then fully quit "
                "the app (menubar → Quit) and relaunch. Verify in System "
                "Settings → Privacy & Security: 'fstt' enabled under "
                "Accessibility, Input Monitoring, AND Microphone."
            )
