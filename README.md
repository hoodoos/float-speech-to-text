...
*   **Переключение моделей:** Выбор между русской моделью (gigaam) и английской (Whisper) через кнопку EN/RU в интерфейсе.
*   **Управление записью:**
...
3.  **EN/RU — переключение модели:** Кнопка переключает между английской (Whisper) и русской (gigaam) моделями распознавания.
4.  **Авто-вставка:** Если включена галочка (☑), текст сразу появится в активном поле ввода.
5.  **Перезапись:** Если вы оговорились, нажмите кнопку перезагрузки (↻) во время записи, чтобы сбросить текущий фрагмент и начать заново.
6.  **Перемещение:** Окно можно перетаскивать мышью за любую область.
